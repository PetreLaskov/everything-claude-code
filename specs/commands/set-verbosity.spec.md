...
2. **Argument handling:**
   ...
   **No argument (`/set-verbosity`):** Display current verbosity setting with an explanation of the scale:
   ```
   Current verbosity: 3
   ...
   **With argument (`/set-verbosity N`):** Set verbosity to N (1-5).
...
2. **Display test.** `/set-verbosity` with no argument shows the current setting and scale description.
3. **Update test.** `/set-verbosity 2` sets `settings.verbosity` to 2 in the learner profile.
